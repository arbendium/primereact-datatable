...
import { useMergeProps, usePrevious } from 'primereact/hooks/hooks.esm.js';
...
export const HeaderCell = React.memo(props => {
	...
	const getStyle = () => {
		...
		return getColumnProp('frozen') ? ({ ...columnStyle, ...headerStyle, ...styleObjectState }) : ({ ...columnStyle, ...headerStyle });
	};
	...
	const createSortIcon = ({ sorted, sortOrder }) => {
		if (getColumnProp('sortable')) {
			...
			const icon = sorted ? sortOrder < 0 ? <SortAmountDownIcon {...sortIconProps} /> : <SortAmountUpAltIcon {...sortIconProps} /> : <SortAltIcon {...sortIconProps} />;
			const sortIcon = IconUtils.getJSXIcon(props.sortIcon || icon, { ...sortIconProps }, { props, sorted, sortOrder });
