...
import { useMergeProps, useUnmountEffect, useUpdateEffect } from 'primereact/hooks/hooks.esm.js';
import { DomHandler, ObjectUtils } from 'primereact/utils/utils.esm.js';
...
export const TableBody = React.memo(
	...
	React.forwardRef((props, ref) => {
		...
		const onSingleSelection = ({
			originalEvent, data, index, toggleable, type
		}) => {
		...
		const onCellClick = event => {
			...
			if (allowCellSelection()) {
				if (allowRangeSelection(event)) {
					...
					if (isSingleSelection()) {
						onSingleSelection({
							originalEvent, data, index: event.rowIndex, toggleable, type: 'cell'
						});
					} else {
						onMultipleSelection({
							originalEvent, data, index: event.rowIndex, toggleable, type: 'cell'
