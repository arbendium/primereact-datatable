...
import { useMountEffect } from 'primereact/hooks/hooks.esm.js';
import { classNames, mergeProps } from 'primereact/utils/utils.esm.js';
...
export const TableHeader = React.memo(props => {
	...
	const createFilterCells = () => React.Children.map(props.columns, (col, i) => {
		...
		if (isVisible) {
			const {
				filterHeaderStyle, style, filterHeaderClassName, className, frozen, columnKey, field, selectionMode, filter
