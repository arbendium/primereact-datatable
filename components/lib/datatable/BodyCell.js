...
export const BodyCell = React.memo(props => {
	...
	const getStyle = () => {
		...
		return getColumnProp('frozen') ? ({ ...columnStyle, ...bodyStyle, ...styleObjectState }) : ({ ...columnStyle, ...bodyStyle });
	};
	...
	const onRowEditInit = event => {
		props.onRowEditInit({
			originalEvent: event, data: props.rowData, newData: getEditingRowData(), field, index: props.rowIndex
		});
	};
	...
	const createElement = () => {
		...
		const columnBodyOptions = {
			column: props.column, field, rowIndex: props.rowIndex, frozenRow: props.frozenRow, props: props.tableProps
		};
		...
		if (selectionMode) {
			...
			if (body) {
				...
				content = ObjectUtils.getJSXElement(body, props.rowData, {
					column: props.column, field, rowIndex: props.rowIndex, frozenRow: props.frozenRow, props: props.tableProps, expander: expanderProps
				});
			}
			...
			if (body) {
				...
				content = ObjectUtils.getJSXElement(body, props.rowData, {
					column: props.column, field, rowIndex: props.rowIndex, frozenRow: props.frozenRow, props: props.tableProps, rowEditor: rowEditorProps
				});
			}
