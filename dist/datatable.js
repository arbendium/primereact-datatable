...
import { useEventListener, useUpdateEffect, useUnmountEffect, useOverlayListener, usePrevious, useMountEffect } from 'primereact/hooks/hooks.esm.js';
...
import { ObjectUtils, classNames, mergeProps, IconUtils, DomHandler, ZIndexUtils, UniqueComponentId } from 'primereact/utils/utils.esm.js';
  ...
  const onEditChange = (e, isEditing) => {
    if (props.onRowEditChange) {
      ...
      if (dataKey) {
        ...
        if (editingRowIndex !== -1) editingRows = editingRows.filter((val, i) => i !== editingRowIndex);else editingRows.push(data);
      }
...
React.forwardRef((props, ref) => {
  ...
  const onSelect = event => {
    if (allowCellSelection()) props.onCellSelect && props.onCellSelect({
      originalEvent: event.originalEvent,
      ...event.data,
      type: event.type
  ...
  const onUnselect = event => {
    if (allowCellSelection()) props.onCellUnselect && props.onCellUnselect({
      originalEvent: event.originalEvent,
      ...event.data,
      type: event.type
...
const ColumnFilter = /*#__PURE__*/React.memo(props => {
  ...
  const showMenuButton = () => getColumnProp('showFilterMenu') && (props.display === 'row' ? getColumnProp('dataType') !== 'boolean' : true);
  ...
  const filterLabel = () => localeOption('filter');
...
const DataTable = /*#__PURE__*/React.forwardRef((inProps, ref) => {
  ...
  const saveColumnWidths = state => {
    const widths = [];
    const headers = DomHandler.find(elementRef.current, '.p-datatable-thead > tr > th');
  ...
  const addColumnWidthStyles = widths => {
    createStyleElement();
    let innerHTML = '';
    const selector = `.p-datatable[${attributeSelector.current}] > .p-datatable-wrapper ${isVirtualScrollerDisabled() ? '' : '> .p-virtualscroller'} > .p-datatable-table`;
...
`;
    ...
    if (!props.lazy) {
      if (data && data.length) {
        ...
        if (localSortField || ObjectUtils.isNotEmpty(localMultiSortMeta)) {
          if (props.sortMode === 'single') data = sortSingle(data, localSortField, localSortOrder);else if (props.sortMode === 'multiple') data = sortMultiple(data, localMultiSortMeta);
        }
      }
    }
    return data;
  }, [props.value, props.lazy, filters, sortField, sortOrder, multiSortMeta, columns, props.globalFilter, filterLocal, props.sortMode, sortSingle, sortMultiple]);
  ...
  const VirtualScrollerContentTemplate = React.useCallback(props => {
    const ref = el => {
      tableRef.current = el;
      props.spacerRef && props.spacerRef(el);
    };
    const _isVirtualScrollerDisabled = isVirtualScrollerDisabled(); // depends on props
    const tableHeader = createTableHeader(props, empty, _isVirtualScrollerDisabled);
    const tableBody = createTableBody(props, selectionModeInColumn, empty, _isVirtualScrollerDisabled);
    const tableFooter = createTableFooter(props);
  ...
  }, [props, empty, selectionModeInColumn, metaData, createTableHeader, createTableBody, createTableFooter, data, columns]);
